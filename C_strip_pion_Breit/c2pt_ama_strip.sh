...
stripping(){
   ...
   src_count=0
   for exsrc in `ls -lh $path_in/c2pt.$cfg.ex.$bxp.*.aff | grep $filesize | awk '{print $9}' | cut -d "." -f5`
   do
      ...
      echo "$path_in/c2pt.$cfg.ex.$bxp.${exsrc}.aff,$cfg_pathout/temp_ex_paths_src${src_count}.txt" >> $cfg_pathout/temp_ex_files.txt
   done
  
   src_count=0
   for slsrc in `ls -lh $path_in/c2pt.$cfg.sl.$bxp.*.aff | grep $filesize | awk '{print $9}' | cut -d "." -f5`
   do
      src_count=`expr $src_count + 1`
      ...
      echo "$path_in/c2pt.$cfg.sl.$bxp.${slsrc}.aff,$cfg_pathout/temp_sl_paths_src${src_count}.txt" >> $cfg_pathout/temp_sl_files.txt
   done
