...
stripping(){
   ...
   src_count=0
   for exsrc in `ls -lh $path_in/qpdf.$cfg.ex.${bxplist[$px]}$hyp.*.PX${px}PY0PZ0dt${ts}.hyp1m140.CG45.aff | grep -v x0y0z0t0 | grep $filesize | awk '{print $9}' | cut -d "." -f5`
   do
   ...
   src_count=0
   for slsrc in `ls -lh $path_in/qpdf.$cfg.sl.${bxplist[$px]}$hyp.*.PX${px}PY0PZ0dt${ts}.hyp1m140.CG45.aff | grep -v x0y0z0t0 | grep $filesize | awk '{print $9}' | cut -d "." -f5`
   do
      src_count=`expr $src_count + 1`
